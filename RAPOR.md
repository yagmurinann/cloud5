...
1. **Launch Template (Başlatma Şablonu):** 
   Sisteme eklenecek her yeni sunucunun temel bir kopyası (Blueprint) niteliğindedir. `t2.micro` sunucu tipi seçilmiş olup, en güncel Amazon Linux 2 AMI ID'si AWS Systems Manager (SSM) üzerinden dinamik olarak çekilmektedir. Şablona eklenen `UserData` bash scripti sayesinde, sunucu her açıldığında Apache web sunucusu (httpd) otomatik kurulur ve "Yagmur Inan E-Ticaret Platformuna Hos Geldiniz" mesajını barındıran anasayfa (index.html) oluşturulur.
...
* **[Gün 2] `commit a7b9e32`: Güvenlik Duvarı ve Launch Template Entegrasyonu**
  ...
  - SSM kullanılarak dinamik AMI çekme işlemi yapıldı.
